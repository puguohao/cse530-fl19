...
public class HeapPage {
	...
	public boolean slotOccupied(int s) {
		...
		int bitNo = s%8;
		int compare = 1 << bitNo;
		int result = compare & header[byteNo];
		if(result == 0)
			return false;
		else 
			return true;
	...
	public void setSlotOccupied(int s, boolean value) {
		...
		if(value == true) {
			i <<= bitNo;
			header[byteNo] = (byte)(header[byteNo] | i);
		}
		else {
			i <<= bitNo;
			i = ~i;
	...
	public void addTuple(Tuple t) throws Exception {
		...
		if(!t.getDesc().equals(this.td))
			throw new Exception("the TupleDesc isn't match!");
		if(haveAvailSlot()) {
			int index = getAvailSlot();
			t.setId(index);
			t.setPid(this.id);
			this.tuples[index] = t;
			...
			throw new Exception("this HeapPage is full!");
	}
	...
	public void deleteTuple(Tuple t) throws Exception{
		//your code here
		if(t.getPid()!=this.id)
			throw new Exception("this tuple is not on this HeapPage!");
		if(slotOccupied(t.getId())) {
			setSlotOccupied(t.getId(), false);
			this.tuples[t.getId()] = null;
		}
		else
			throw new Exception("this tuple slot is already empty!");
	...
	public int getAvailSlot() throws Exception{
		if(!haveAvailSlot())
			throw new Exception("this HeapPage is full!");
		int i = 0;
		for(; i<getNumSlots(); i++)
